...
# Carregar CSV
try:
    # Assumindo que não há cabeçalho no CSV original, então header=None
    # Se o seu CSV tiver cabeçalho, remova header=None ou ajuste-o.
    df = pd.read_csv('dados_recebidos.csv', header=None)
...
# Renomear colunas (ajuste se seu CSV já tiver cabeçalhos corretos)
df.columns = ['Ciclo', 'Fonte', 'ID', 'T1', 'T2', 'T3', 'T4']
...
# Função para gerar gráfico de linha por ciclo
# Esta função usa os nomes de coluna CORRETOS: 'T1->T2', 'T2->T3', 'T3->T4', 'Tempo Total (T1->T4)'
def plot_por_ciclo(ciclo_num, dataframe_local): # Renomeado para dataframe_local para evitar confusão com df global
    ...
    # ESTA É A LINHA QUE CAUSAVA O ERRO NO SEU SCRIPT. AGORA ESTÁ CORRIGIDA:
...
# Gráficos de linha por ciclo (usando a função corrigida)
# Se o seu traceback indicava `plot_por_ciclo(0)` na linha 45,
# você pode usar chamadas explícitas se preferir, mas o loop é mais geral.
ciclos_unicos = df['Ciclo'].unique()
if len(ciclos_unicos) > 0:
    for ciclo_iter in ciclos_unicos: # Renomeado para ciclo_iter para não conflitar com a coluna 'Ciclo'
...
if 'T1' in df.columns:
    ...
    df.dropna(subset=['T1_datetime'], inplace=True) # Remover se a conversão falhou

    if not df.empty:
        plt.figure()
        # Usar uma coluna que sempre existe para contar, como 'ID' ou qualquer outra.
        # 'size()' é o método correto para resample.
        ...
        if any(df['Ciclo'] == ciclo_iter for ciclo_iter in ciclos_unicos): # Verifica se algum plot foi feito
...
# --- ANÁLISE 6: Diferença de tempo entre LoadBalancers em cada requisição (assumindo ID pareado) ---
...
if all(col in df.columns for col in ['ID', 'Tempo Total (T1->T4)', 'LoadBalancer']):
    ...
    try:
        if pd.api.types.is_numeric_dtype(df['ID']):
            ...
            try: # Tentar converter ID para string para garantir ordenação consistente em categóricos
            ...
            except: # Fallback se a conversão ou ordenação falhar
                sns.lineplot(data=df, x='ID', y='Tempo Total (T1->T4)', hue='LoadBalancer', marker='.', errorbar='sd')
